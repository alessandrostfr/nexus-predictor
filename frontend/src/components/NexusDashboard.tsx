  ...
  return Array.from(grouped.entries())
    .map(([slug, roomSlots]) => {
      ...
      return {
        ...
        peakTime: peak ? `${eventDayLabel(peak.event_day)} · ${peak.start_time}` : '—',
...
function eventDayLabel(value?: string | null) {
  if (!value || value === 'nexus_day') return 'Nexus Day · 12:00-06:00';
  if (value === 'all') return 'Evento continuo completo';
  return title(value);
}
    ...
    <section className="page-block">
      ...
      <div className="toolbar-card">
        ...
        <select className="select-control" value={selectedDay} onChange={(event) => setSelectedDay(event.target.value)}>
          <option value="all">Evento continuo completo</option>
          <option value="nexus_day">Nexus Day · 12:00-06:00</option>
        </select>
      </div>
      ...
      <div className="schedule-columns">
        {Object.entries(grouped).map(([group, slots]) => (
          <article className="card schedule-column" key={group}>
            <div className="card-header">
              <div>
                <small>{eventDayLabel(slots[0]?.event_day)}</small>
            ...
            <ul className="slot-feed compact-feed">
              {artistSlots.slice(0, 8).map((slot) => (
                <li key={`${slot.variant_key}-${slot.artist_slug}-${slot.start_time}`}>
                  <time>{slot.start_time}</time>
                  <div><strong>{slot.variant_name ?? scheduleLabels[(slot.variant_key as ScheduleKey) ?? 'probable']}</strong><span>{eventDayLabel(slot.event_day)} · {slot.room_name}</span></div>
                  <span className={`pressure-badge ${riskTone(pressureFromSlot(slot))}`}>{normalizedPercent(pressureFromSlot(slot))}</span>
                </li>
              ))}
            </ul>
